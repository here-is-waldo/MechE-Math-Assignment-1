...
% end
%% 
function Assignemnt1
    x_range = linspace(-10,38,200);

    [y_vals,dydx] = test_func(x_range);
    ...
    semilogy(abs(x_guess_list1-x_root1));

    figure(3);
    loglog(all_error_current_list,all_error_next_list)
end

function [x_root,exit_flag,x_guess_list1] = newton_solve(f_in,x0,max_iter,dx_tol,y_tol)
    x_prev = x0-2*dx_tol;    
    x_val = x0;
...
function [f_val, dfdx] = test_func(x_range)
    f_val = @(x_range) (x_range.^3)/100 - (x_range.^2)/8 + 2*x_range + 6*sin(x_range/2+6) -.7 - exp(x_range/6);
    dfdx = @(x_range) 3*(x_range.^2)/100 - 2*x_range/8 + 2 +(6/2)*cos(x_range/2+6) - exp(x_range/6)/6;
end